...
這不是「往子結構走」，而是**換一個 nonterminal 的眼光重看同一個值**——
文法說 `S-exp` 可以整個就是一個 `S-list`。
...
這是**可除錯性**（debuggability）的保證，不是正確性的保證。

錯了還是會錯——但你知道要去哪一格找。

> 對照你日常的工作：這正是「大型重構之所以可行」的理由。
...
2. follow the grammar 保證的是**可除錯性**，不是正確性
...
Clojure 只是把那個結構**另外提供一層**，而不是要求資料本身具備。
